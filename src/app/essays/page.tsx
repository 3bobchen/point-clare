...
function PictureLink(props: PictureLinkProps) {
  const { img, link, name, subtitle, tag, credit } = props;
  return (
    <div className="group flex w-full py-3">
      ...
      >
        <div className="absolute bg-white text-gray-600 px-2 rounded-lg left-3 top-3">
          {tag}
        </div>
        ...
        <div className="flex flex-col self-start w-full p-5 justify-between h-full">
          <div>
            ...
            <Suspense fallback={<p className="h-6" />}>{subtitle}</Suspense>
          </div>
