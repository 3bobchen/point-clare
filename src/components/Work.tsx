...
export function WorkHistoryObject(props: { work: WorkDataObject }) {
  ...
  const total_months2 =
    (work.endYear ?? new Date().getFullYear()) * 12 +
    (work.endMonth ?? new Date().getMonth());
  ...
  return (
    <div className="group flex-col w-full py-3">
      <div className="relative border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 rounded flex-col w-full">
        <div className="flex items-center justify-between">
          <div className="absolute bg-white text-gray-600 px-2 rounded-lg right-3 top-3">{`${year_difference != 0 ? year_difference : ""} ${year_difference != 0 ? (year_difference > 1 ? "years" : "year") : ""} ${month_difference} ${month_difference > 1 ? "months" : "month"}`}</div>
          <div className="px-6">
            <Image src={work.companyLogo} alt="" width={300} height={100} />
          </div>
          <div className="flex flex-col self-start w-full p-5 justify-between h-full">
            <div>
              ...
              <p className="text-neutral-900">{work.location}</p>
              ...
              <div className="my-5 mx-8">
                ...
                {role.oneOff ? (
                  <p className="text-neutral-500">
                    {months[role.roleStartMonth - 1]} {role.roleStartYear}
                  </p>
                ) : (
                  <p className="text-neutral-500">
                    {months[role.roleStartMonth - 1]} {role.roleStartYear} -{" "}
                    {role.roleEndMonth ? months[role.roleEndMonth - 1] : null}{" "}
                    {role.roleEndYear ?? "Present"}
                  </p>
                )}
